...
-   [CDN结合WAF](intl.zh-CN/用户指南/接入WAF/CDN结合WAF.md#)：介绍同时为网站部署CDN和Web应用防火墙的配置方法。
-   [高防IP结合WAF](intl.zh-CN/用户指南/接入WAF/高防IP结合WAF.md#)：介绍同时为网站部署DDoS高防和Web应用防火墙的配置方法。
...
-   获取WAF CNAME地址。
    ...
    3.  在**管理** \> **网站配置**页面，选择已添加的网站配置，将鼠标放置在域名上，即可出现**复制CName**按钮。
        ...
        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/15546/15438227557565_zh-CN.png)
...
-   （可选）放行WAF回源段IP。源站服务器上已启用非阿里云安全软件（如安全狗、云锁）时，您需要在这些软件上设置放行WAF回源段IP，防止由WAF转发到源站的正常业务流量被拦截。具体请参考[放行WAF回源段IP](../intl.zh-CN/用户指南/放行WAF回源IP段.md#)。
-   （可选）进行本地验证。通过本地验证确保WAF转发规则配置正常后，再修改网站域名的DNS解析记录，防止因配置错误导致业务中断。具体请参考[本地验证](../intl.zh-CN/用户指南/本地验证.md#)。
...
2.  选择要操作的域名，单击其操作列下的**解析设置**。
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/15549/15438227557588_zh-CN.jpg)
...
3.  选择要操作的**主机记录**，单击其操作列下的**修改**。
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/15549/15438227557589_zh-CN.jpg)
...
4.  在修改记录对话框中，完成以下操作：
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/15549/15438227557590_zh-CN.jpg)
...
-   **前提条件**：获取WAF CNAME后，执行以下步骤，获取WAF IP地址。
    ...
    2.  执行以下命令：`ping "已复制的WAF Cname地址"`。
        ...
        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/15553/154382275632229_zh-CN.png)
