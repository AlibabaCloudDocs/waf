...
-   [Web应用防火墙是否支持接入采用NTLM协议认证的网站防护？](#section_wgn_qt3_mgb)
...
Web应用防火墙提供不同的CC安全防护模式：正常和攻击紧急。您可以根据实际情况进行选择。具体请参考[选择CC防护模式](../../../../../cn.zh-CN/用户指南/防护配置/CC安全防护.md#)。

如果您希望同时有好的防护效果和低误杀率，建议您选择Web应用防火墙企业版和旗舰版，自定义或让安全专家为您定制针对性的防护算法。配置方法请参考[自定义CC防护](../../../../../cn.zh-CN/用户指南/防护配置/自定义CC防护.md#)。
...
**说明：** 不是任意端口都支持自定义，非标准端口必须在支持范围内，详情请参考[非标端口支持](../../../../../cn.zh-CN/用户指南/使用DNS配置模式接入WAF/非标端口支持.md#)。
...
Web应用防火墙所有版本都支持短信防刷。关于Web应用防火墙各版本功能，请查看[Web应用防火墙各版本功能介绍](../../../../../cn.zh-CN/产品定价/开通WAF/WAF各版本功能说明.md#)。
...
具体配置方法，请查看[高防IP结合WAF](../../../../../cn.zh-CN/用户指南/使用DNS配置模式接入WAF/同时部署WAF和DDoS高防.md#)及[CDN结合WAF](../../../../../cn.zh-CN/用户指南/使用DNS配置模式接入WAF/同时部署WAF和CDN.md#) 。
...
**说明：** 源站IP无法响应时，Web应用防火墙将为该源站IP自动设置一个静默时间。静默时间结束后，新的访问请求可能仍然会被转发至该源站IP。关于WAF的健康检查工作原理，参考SLB服务的[健康检查原理](../../../../../cn.zh-CN/历史文档/用户指南（旧版控制台）/监听/健康检查/健康检查介绍.md#)。
...
您可以登录[云盾Web应用防火墙控制台](https://yundun.console.aliyun.com/?p=waf) 中，在**管理** \> **网站配置**页面查看详细的Web应用防火墙回源IP段。详情请参考[如何查看Web应用防火墙回源IP段](cn.zh-CN/常见问题/如何查看Web应用防火墙回源IP段？.md#)。
...
您可以参考[源站保护](../../../../../cn.zh-CN/最佳实践/源站保护.md#)，对您的源站进行安全防护配置。

## Web应用防火墙回源是否需要放行所有客户端IP？ {#section_kgb_j5k_q2b .section}

根据您的业务情况，您可以只放行WAF回源IP段，也可以放行所有客户端IP。

对于Web业务，建议您只放行WAF回源IP，实现[源站保护](../../../../../cn.zh-CN/最佳实践/源站保护.md#)。
...
-   Web应用防火墙的黑洞阈值和当前地区ECS的默认阈值相同，且支持通过[安全信誉防护联盟](https://help.aliyun.com/document_detail/40054.html)提升。

DDoS攻击可以通过购买[DDoS高防IP](https://www.aliyun.com/product/ddos)服务进行防护。
...
## Web应用防火墙是否支持接入采用NTLM协议认证的网站防护？ {#section_wgn_qt3_mgb .section}
