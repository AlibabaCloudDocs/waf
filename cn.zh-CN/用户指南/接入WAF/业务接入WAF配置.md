# 业务接入WAF配置 {#concept_ngy_ws3_p2b .concept}
...
-   [同时部署WAF和CDN](cn.zh-CN/用户指南/接入WAF/同时部署WAF和CDN.md#)：介绍同时为网站部署CDN和Web应用防火墙的配置方法。
-   [同时部署WAF和DDoS高防](cn.zh-CN/用户指南/接入WAF/同时部署WAF和DDoS高防.md#)：介绍同时为网站部署DDoS高防和Web应用防火墙的配置方法。
...
-   获取WAF CNAME地址。
    ...
    3.  在**管理** \> **网站配置**页面，选择已添加的网站配置，将鼠标放置在域名上，即可出现**复制CName**按钮。
        ...
        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/15546/15482239137565_zh-CN.png)

    4.  单击**复制CName**，将该CNAME复制到剪贴板中。

        **说明：** 若需要使用A记录接入WAF，您可以Ping该CNAME地址，获取对应的WAF IP。参照[WAF接入指南](../cn.zh-CN/用户指南/接入WAF/业务接入WAF配置.md#)进行后续操作。一般情况下，防护该域名的WAF IP不会频繁变更。

-   具有在域名的DNS服务商处更新DNS记录的权限。
-   （可选）放行WAF回源段IP。源站服务器上已启用非阿里云安全软件（如安全狗、云锁）时，您需要在这些软件上设置放行WAF回源段IP，防止由WAF转发到源站的正常业务流量被拦截。具体请参考[放行WAF回源段IP](cn.zh-CN/用户指南/接入WAF/放行WAF回源IP段.md#)。
-   （可选）进行本地验证。通过本地验证确保WAF转发规则配置正常后，再修改网站域名的DNS解析记录，防止因配置错误导致业务中断。具体请参考[本地验证](cn.zh-CN/用户指南/接入WAF/本地验证.md#)。
...
2.  选择要操作的域名，单击其操作列下的**解析设置**。
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/15549/15482239137588_zh-CN.jpg)
...
3.  选择要操作的**主机记录**，单击其操作列下的**修改**。
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/15549/15482239137589_zh-CN.jpg)
...
4.  在修改记录对话框中，完成以下操作：
    ...
    -   如果必须保留MX记录（邮件服务器记录），您可以参考[WAF接入指南](../cn.zh-CN/用户指南/接入WAF/业务接入WAF配置.md#)，使用A记录解析的方式将域名解析到WAF IP。
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/15549/15482239137590_zh-CN.jpg)
    ...
    2.  前往**管理** \> **网站配置**页面，查看域名的**DNS解析状态**。
        ...
        -   **异常**：如果DNS解析状态为异常，且收到**未检测到CNAME接入**、**无流量**、**检测失败**等提示，说明网站未正确接入WAF。
            ...
            **说明：** 该提示仅说明网站是否正确接入WAF，不代表您的网站访问异常。
            ...
            ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/15549/15482239137591_zh-CN.jpg)
...
![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/15553/154822391332207_zh-CN.jpg)
...
-   **前提条件**：获取WAF CNAME后，执行以下步骤，获取WAF IP地址。
    ...
    2.  执行以下命令：`ping "已复制的WAF Cname地址"`。
        ...
        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/15553/154822391332229_zh-CN.png)
