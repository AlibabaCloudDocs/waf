...
|可选以下由WAF指定的非标端口中的任意50个：-   HTTP协议端口：
...
|透明接入和CNAME接入模式下，可选的非标端口不同。说明如下：-   **透明接入模式**：支持0~65535范围内的任意50个非标端口。
-   **CNAME接入模式**：支持以下由WAF指定的非标端口中的任意50个：
    -   HTTP协议端口：
...
**说明：** 仅中国内地地域的WAF实例支持18980端口，海外地区WAF实例暂不支持18980端口。 |
|独享版|50个|-   HTTP：80、8080
-   HTTPS：443、8443

|支持0~65535范围内的任意50个非标端口。|
