...
|SLB地域|您的公网SLB实例地域必须位于华北2（北京）、华东1（杭州）、华东2（上海）、华南1（深圳）或西南1（成都）。**说明：** 后续将陆续开放支持其他地域。
...
|域名备案|需要防护的域名网站如果托管在中国内地（大陆）的服务器上，该域名需要完成ICP备案。|无|
|证书状态|透明接入配置的端口使用的证书必须都已上传到阿里云SSL证书服务进行统一管理。|证书未上传到阿里云证书服务中，SLB无法将该证书自动同步到WAF中，您将无法完成网站接入。相关内容请参见[上传已有证书](/cn.zh-CN/证书管理/上传已有证书.md)。|
...
域名端口首次透明接入WAF时，可能会导致Web业务出现秒级闪断。您可以在总览页面查看到当前业务QPS有明显下降。
...
5.  在添加域名页面，完成添加域名的配置。
    ...
    ![首次接入授权](https://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/zh-CN/5642833061/p175894.png)
